...
class Usuario
{
    ...
    {
            $users = Usuario::_cargarListado();
            if(Usuario::UsuarioRegistrado($u, $users))
            {   
                if(Usuario::VerificaClave($u, $users))
                {
                    echo "Login Exitoso";
                }
                else{
    ...
    static function UsuarioRegistrado($u, $users)
    {
        foreach ($users as $key => $value) {
            if(strcmp($value->mail, $u->mail) == 0)
            {
                return TRUE;
            }
        }
    ...
    static function VerificaClave($u, $users)
    {
        foreach ($users as $key => $value) {
            if(strcmp($value->mail, $u->mail) == 0  && strcmp($value->clave, $u->clave) == 0)
            {
                return TRUE;
            }
        }
    ...
    {   
        ...
        if ($miArchivo)
        {
            while(($linea = fgets($miArchivo, 512)) != FALSE)
            {
                ...
                array_push($retorno, new Usuario($userAux[0], $userAux[1], trim($userAux[2])) );
            }
            return $retorno;
        }
        return NULL;
